...
from azllm.clients.openai import OpenAIClient, DEFAULT_CONFIG 
...
def test_get_default_config():
    assert OpenAIClient.get_default_config() == DEFAULT_CONFIG

# ------------------------
# 2. Test Initialization
# ------------------------

def test_openai_client_init_defaults():
    client = OpenAIClient()
...
def test_openai_client_init_custom():
    custom_config = {
        'model': 'gpt-4',
        'parameters': {
...
def test_get_api_key_missing(monkeypatch):
    ...
    client = OpenAIClient()
    with pytest.raises(ValueError, match="A valid API Key for OpenAI is missing"):
        client.get_api_key()
...
@patch("azllm.clients.openai.OpenAI")  
def test_generate_text_mocked(mock_openai):
    ...
    mock_openai.assert_called_once_with(api_key="mock-key")
