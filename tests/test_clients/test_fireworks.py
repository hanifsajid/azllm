...
from azllm.clients.fireworks import FireworksClient, DEFAULT_CONFIG 
...
def test_get_default_config():
    assert FireworksClient.get_default_config() == DEFAULT_CONFIG

# ------------------------
# 2. Test Initialization
# ------------------------

def test_fireworks_client_init_defaults():
...
def test_fireworks_client_init_custom():
    custom_config = {
    ...
    client = FireworksClient(config=custom_config)
    assert client.model == 'accounts/fireworks/models/llama4-scout-instruct-basic'
...
def test_get_api_key_missing(monkeypatch):
    ...
    client = FireworksClient()
    with pytest.raises(ValueError, match="A valid API Key for Fireworks is missing."):
        client.get_api_key()
...
@patch("azllm.clients.fireworks.OpenAI")  
def test_generate_text_mocked(mock_openai):
    ...
    mock_openai.assert_called_once_with(api_key="mock-key", base_url="https://api.fireworks.ai/inference/v1")
