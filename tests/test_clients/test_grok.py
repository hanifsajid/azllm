...
from azllm.clients.grok import GrokClient, DEFAULT_CONFIG 
...
def test_get_default_config():
    assert GrokClient.get_default_config() == DEFAULT_CONFIG
...
def test_grok_client_init_custom():
    ...
    client = GrokClient(config=custom_config)
...
def test_get_api_key_missing(monkeypatch):
    ...
    client = GrokClient()
    with pytest.raises(ValueError, match="A valid API Key for Grok is missing."):
        client.get_api_key()
...
@patch("azllm.clients.grok.OpenAI")  
def test_generate_text_mocked(mock_openai):
    ...
    mock_openai.assert_called_once_with(api_key="mock-key", base_url="https://api.x.ai/v1")
